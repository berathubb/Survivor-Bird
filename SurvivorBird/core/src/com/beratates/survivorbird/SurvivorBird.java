package com.beratates.survivorbird;
		...
		if(gameState == 1){
			...
			for(int i = 0; i < numberOfEnemies; i++){
				...
				if(enemyX[i] < 0){
					...
					enemyOffSet[i] = (random.nextFloat() - 0.5f) * (Gdx.graphics.getHeight() - 270);
					enemyOffSet2[i] = (random.nextFloat() - 0.5f) * (Gdx.graphics.getHeight() - 280);
					enemyOffSet3[i] = (random.nextFloat() - 0.5f) * (Gdx.graphics.getHeight() - 290);
			...
			font2.draw(batch,"Oyun Bitti! Tekrar Oynamak Için Dokunun", 100,Gdx.graphics.getHeight()/2);
			if(Gdx.input.justTouched()){
				gameState = 1;
